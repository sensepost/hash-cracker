...
if [[ $USERULES =~ ^[Yy]$ ]]; then
    ...
    $HASHCAT $KERNEL --bitmap-max=24 $DEVICE $HWMON $SHOWCRACKED --potfile-path=$POTFILE -m$HASHTYPE $HASHLIST $tmp2 -r $RULE $LOOPBACK
    done
    rm $tmp2
elif [[ $USERULES =~ ^[Nn]$ ]]; then
    $HASHCAT $KERNEL --bitmap-max=24 $DEVICE $HWMON $SHOWCRACKED --potfile-path=$POTFILE -m$HASHTYPE $HASHLIST $tmp2
    rm $tmp2
else
    echo -e "Try again...\n"
fi

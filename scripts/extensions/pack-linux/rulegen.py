...
class RuleGen:
    ...
    def __init__(self,language="en",providers="aspell,myspell",basename='analysis',threads=multiprocessing.cpu_count()):
        ...
        if enchant is None:
            raise SystemExit("[!] Missing dependency: install pyenchant to use rulegen.py")
    ...
    def analyze_passwords_file(self,passwords_file):
        ...
        # Setup queues
        passwords_queue = multiprocessing_context.Queue(self.threads)
        rules_queue = multiprocessing_context.Queue()
        words_queue = multiprocessing_context.Queue()

        # Start workers
        for i in range(self.threads):
            multiprocessing_context.Process(target=self.password_worker, args=(i, passwords_queue, rules_queue, words_queue)).start()
        multiprocessing_context.Process(target=self.rule_worker, args=(rules_queue, "%s.rule" % self.basename)).start()
        multiprocessing_context.Process(target=self.word_worker, args=(words_queue, "%s.word" % self.basename)).start()
